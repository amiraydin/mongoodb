...
//99 users
// Création des villes issues du tableau `cities` dans la base de données
...
cityModel.aggregate()
    .match({ // Filtrer
        city: { // Autre possibilité de filtre: /^p/i
            $regex: '^p',
            $options: 'i'
        }
    })
    .group({ // Regrouper
        ...
        populationTotale: { // Somme de la population totale par département
            $sum: '$population'
        },
        populationMoyenne: { // Population moyenne par département
            $avg: '$population'
        },
        cities: { // Somme des villes par département
            $sum: 1
        }
    })
    .sort({ // Trier
        populationTotale: 1 // Tri ascendant par population totale des département
    })
    .then(console.log)
    .catch(console.error)
